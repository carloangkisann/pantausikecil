...
import { View, Text, TextInput, TouchableOpacity, ScrollView, Alert, StatusBar, Image } from 'react-native';
...
export default function BuatKehamilan() {
  ...
  const saveKehamilan = async () => {
    try {
      // Validasi form
      if (!formData.kehamilanKe || !formData.usiaKehamilan || !formData.jenisKelaminBayi || !formData.komplikasiKehamilan) {
        Alert.alert('Error', 'Mohon lengkapi semua field yang diperlukan');
        return;
      }

      // Mock save - langsung redirect ke tabs
      Alert.alert('Berhasil', 'Data kehamilan berhasil disimpan', [
        { text: 'OK', onPress: () => router.replace('/(tabs)') }
      ]);
    } catch (error) {
      Alert.alert('Error', 'Gagal menyimpan data kehamilan');
    }
  };
  ...
  return (
    ...
    {/* Jenis Kelamin Bayi */}
        <View className="mb-6">
          ...
          <View className="bg-transparent rounded-xl shadow-sm border border-pink-semi-low" style={{ width: 350, height: 52 }}>
            <Picker

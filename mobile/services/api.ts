...
class ApiService {
  ...
  async login(credentials: LoginRequest): Promise<AuthResponse> {
    try {
      ...
      if (data.success && data.data?.user) {
        try {
          const profileResponse = await this.makeRequest<UserProfile>(`/users/${data.data.user.id}/profile`);
  ...
  async register(userData: RegisterRequest): Promise<AuthResponse> {
    try {
      console.log('Making register request to:', `${this.baseURL}/auth/register`);
  ...
  async updateUserProfile(userId: number, profileData: UpdateProfileRequest): Promise<ApiResponse<UserProfile>> {
    console.log('Making update profile request to:', `${this.baseURL}/users/${userId}/profile`);
  ...
  // ===== PREGNANCY METHODS =====
  async createPregnancy(userId: number, pregnancyData: CreatePregnancyRequest): Promise<ApiResponse<PregnancyData>> {
  ...
  async getUserPregnancies(userId: number): Promise<ApiResponse<PregnancyData[]>> {
    return this.makeRequest(`/users/${userId}/pregnancies`);
  }

  async updatePregnancy(userId: number, pregnancyId: number, pregnancyData: UpdatePregnancyRequest): Promise<ApiResponse<PregnancyData>> {
  ...
  // ===== CONNECTION METHODS =====
  async getUserConnections(userId: number): Promise<ApiResponse<UserConnection[]>> {
    return this.makeRequest(`/users/${userId}/connections`);
  }

  async createConnection(userId: number, connectionData: CreateConnectionRequest): Promise<ApiResponse<UserConnection>> {
    return this.makeRequest(`/users/${userId}/connections`, {
      method: 'POST',
  ...
  async deleteConnection(userId: number, connectionId: number): Promise<ApiResponse<void>> {
  ...
  // ===== REMINDER METHODS =====
  async createReminder(userId: number, reminderData: CreateReminderRequest): Promise<ApiResponse<UserReminder>> {
  ...
  async getUserReminders(userId: number, date?: string): Promise<ApiResponse<UserReminder[]>> {
    const url = date ? `/users/${userId}/reminders?date=${date}` : `/users/${userId}/reminders`;
    return this.makeRequest(url);
  }

  async getUpcomingReminders(userId: number): Promise<ApiResponse<UserReminder[]>> {
    return this.makeRequest(`/users/${userId}/reminders/upcoming`);
  }

  async deleteReminder(userId: number, reminderId: number): Promise<ApiResponse<void>> {
  ...
  // ===== NUTRITION METHODS =====
  async getNutritionalNeeds(userId: number): Promise<ApiResponse<NutritionalNeeds>> {
    return this.makeRequest(`/users/${userId}/nutrition/needs`);
  }

  async getTodayNutrition(userId: number): Promise<ApiResponse<DailyNutritionSummary>> {
    return this.makeRequest(`/users/${userId}/nutrition/today`);
  }

  async getNutritionSummary(userId: number, date: string): Promise<ApiResponse<DailyNutritionSummary>> {
    return this.makeRequest(`/users/${userId}/nutrition/summary?date=${date}`);
  }

  async getUserMeals(userId: number, date?: string): Promise<ApiResponse<any[]>> {
    const url = date ? `/users/${userId}/nutrition/meals?date=${date}` : `/users/${userId}/nutrition/meals`;
    return this.makeRequest(url);
  }

  async addMeal(userId: number, mealData: AddMealRequest): Promise<ApiResponse<any>> {
  ...
  async addWaterIntake(userId: number, waterData: AddWaterRequest): Promise<ApiResponse<any>> {
  ...
  // ===== ACTIVITY METHODS =====
  async getTodayActivities(userId: number): Promise<ApiResponse<UserActivitySummary>> {
    return this.makeRequest(`/users/${userId}/activities/today`);
  }

  async getActivitySummary(userId: number, date: string): Promise<ApiResponse<UserActivitySummary>> {
    return this.makeRequest(`/users/${userId}/activities/summary?date=${date}`);
  }

  async getRecommendedActivities(userId: number): Promise<ApiResponse<ActivityItem[]>> {
  ...
  async addActivity(userId: number, activityData: AddUserActivityRequest): Promise<ApiResponse<any>> {
  ...
  async removeActivity(userId: number, activityId: number): Promise<ApiResponse<void>> {
  ...
  async getActivityDetails(activityId: number): Promise<ApiResponse<ActivityItem>> {
    return this.makeRequest(`/activities/${activityId}`);
  }

  async calculateCalories(data: { weight: number; duration: number; activityType: string }): Promise<ApiResponse<{ calories: number }>> {
  ...
  // ===== EMERGENCY METHODS =====
  async sendEmergencyNotification(userId: number, emergencyData: EmergencyRequest): Promise<ApiResponse<any>> {
